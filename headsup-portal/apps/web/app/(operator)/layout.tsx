/* Operator Surface ("Virtual GM" under HeadsUP OS) — ecosystem v4 editorial
   layout: masthead card with gradient stripe, horizontal nav, ticker overlay.
   Middleware routes by role first; this check is defense in depth. Data
   visibility (Activation Lock) is enforced at RLS, never here. */
...
export default async function OperatorLayout({ children }: { children: React.ReactNode }) {
  ...
  if (!roleMayEnter(auth.role, "operator")) redirect(homeForRole(auth.role));
        ...
        <div className="flex flex-wrap items-end justify-between gap-4 px-6 pb-4 pt-5">
          <div>
            ...
            <div className="mt-1 font-display text-3xl font-extrabold uppercase leading-none tracking-wide text-ink">
              Virtual <span className="text-hblue">GM</span>
            </div>
            <div className="mt-1 text-xs text-slate-400">
              Operator Surface · We Scout From The Neck Up · Dallas, TX
            </div>
          </div>
